...
- CLI：三个脚本 `--help`、`run_episode` JSON 输出、缺失 config、replay 缺失文件、batch not implemented、`--max-episodes` 边界和不依赖 FastAPI app。
- Acceptance：OpenAPI 全路径、API -> runner -> state/summary/download、WebSocket schema、多次 state 请求不推进 runner、统一错误结构。

## 未覆盖或有意降级

- 真实 `EpisodeRunner` dependency assembly：当前 Module M 使用可注入 `runner_factory`，真实 A-J-L 组装留给后续更完整的服务启动配置。
