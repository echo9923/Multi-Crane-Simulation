...
This document records the Phase 1 developer workflow for the desktop workbench. Phase 1 is a local development workbench with a Windows `win-unpacked` developer package path, not a final signed installer or frozen backend release. Development startup expects the repository Python virtualenv and frontend dependencies to already exist; the Windows package currently copies the local `.venv` into the app resources.

## Scope

Phase 1 connects the existing simulation stack into a desktop-oriented workbench:

- During development, Electron starts the FastAPI backend from the repository `.venv`; packaged Windows builds use the copied resource `.venv` unless `MULTI_CRANE_PYTHON` is set.
...
- The backend and frontend still run as development services during `desktop:dev`. The Windows packaging path is for local unsigned `win-unpacked` verification; installer signing, notarization, auto-update, and backend binary freezing are outside Phase 1.
...
Electron resolves the project root, chooses an available backend port, starts FastAPI with the repository virtualenv Python (`.venv/Scripts/python.exe` on Windows), passes the selected port through `MULTI_CRANE_BACKEND_PORT`, waits for `/health`, then loads the Vite dev server with desktop API and WebSocket runtime parameters. FastAPI enables narrowly scoped CORS for local renderer origins such as `http://127.0.0.1:5173` and `http://localhost:5173`; arbitrary remote origins are not allowed.
...
`desktop:pack` builds an unsigned unpacked Windows app under `frontend/release/win-unpacked/Multi Crane Workbench.exe`. `desktop:dist` uses the same Windows `dir` target today, so it remains an unpacked Windows directory unless installer targets are added later.

The packaged app resolves Python as documented in [desktop_packaging.md](desktop_packaging.md): `MULTI_CRANE_PYTHON`, then packaged `resources/.venv/Scripts/python.exe`, then optional `MULTI_CRANE_DEV_PROJECT_ROOT/.venv/Scripts/python.exe` for packaged-app troubleshooting; ordinary development runs use the checkout `.venv/Scripts/python.exe` on Windows. This is not yet a frozen backend binary; the package remains tied to a compatible Windows/Python virtualenv from the build environment.
