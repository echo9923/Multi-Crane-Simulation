...
本任务负责“来源、拆分、合并和来源诊断”。字段结构、枚举和基础数值校验由 Task 01 的 Pydantic schema 完成；resolved 默认值、seed 派生和 hash 由 Task 03 完成。
...
6. 加载器为每类 typed config 保留 `source_file`、`config_kind` 等来源 metadata，供 Task 06 转换错误对象使用；这些 metadata 不写入 Task 01 的纯配置 schema。
...
- override 只能产生“最终原始字典”，随后交给 Task 01 schema 解析；override 本身不得绕过 schema 校验。

路径处理规则：

- `scenario_ref`、`replay_file`、`run_root` 等路径必须规范化。
- 相对路径以项目根目录或调用方声明的 config root 为基准。
- 输入引用路径不得通过 `..` 穿越到允许的 config root 之外。
- `output.run_root` 在本任务只做规范化和基础安全检查，不创建目录；每次运行的最终 run workspace 内部路径约束由 Task 04 执行。
- 禁止把任何配置路径解析到调用方未授权的读写位置。
...
- 路径规范化只负责来源安全和一致性，不负责创建 run 目录，也不负责检查最终 run workspace 内部文件写入；run 目录和内部写路径由 Task 04 负责。
- 加载器不读取环境变量中的真实 key；密钥解析和脱敏由 Task 05 负责。
- 加载器可以发现 demo config 中出现 `api_key` 字段并记录诊断，但真实 key 判定和安全阻断由 Task 05 负责。
...
- schema 校验失败：保留 Pydantic error 和 source metadata，交给 Task 06 映射为 `CFG_E_001`、`CFG_E_002` 或 dataset config startup error。
...
- CLI override 后的最终字典仍会经过 Task 01 schema 校验。
