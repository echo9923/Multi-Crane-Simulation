...
> 本修订版在 v0.3 基础上继续补齐“可分模块开工”的实施合同：领域对象 schema 生成规则、统一错误码、事件生命周期、任务失败恢复、replay 容差、数据质量门禁、API 错误响应、dataset 风险比例目标、M0-M5 里程碑退出条件和后续待完善清单。  
> 若本文后续章节仍出现旧写法，以“0.5 已确认设计决策”“0.6 本轮修订统一约定”“0.7 工程合同统一约定”和“0.8 实施级补充合同”为准。
